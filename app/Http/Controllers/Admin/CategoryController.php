...
class CategoryController extends Controller
{
    ...
    public function category_store(CategoryCreateRequest $request) {
        ...
        return redirect()->back()->with('message', '追加しました！')->with('title', 'カテゴリー編集')->with('type', 'dialog');
    }
    ...
    public function category_update($category_id, CategoryUpdateRequest $request) {
        ...
        return redirect()->back()->with('message', '保存しました！')->with('title', 'カテゴリー編集')->with('type', 'dialog');
    }

    public function category_destroy($category_id = null) {
        Category::where("id", $category_id)->delete();
        return redirect()->back()->with('message', '削除しました！')->with('title', 'カテゴリー編集')->with('type', 'dialog');
    }
