    ...
    <AppLayout :closeModal="closeModal">
        <div class="mt-3 px-5">  
            ...
            <div class="flex justify-between items-center mb-2">
                <h1 class="text-lg font-bold">カード設定</h1>
            ...
            <div class="flex items-center gap-2 w-full flex-wrap mb-2 justify-between border p-2 text-sm">
                <div class="flex flex-col gap-2 flex-1">
                    <div class="flex flex-wrap gap-y-2">
                        <div class="flex-1 flex items-center gap-2 min-w-[16rem]">
                        ...
                        <div class="flex-1 flex items-center gap-2 min-w-[16rem]">
                            ...
                            <select v-model="form_search.rare" id="last_rare"
                                class="h-9 flex-1 w-0 border-neutral-300 focus:border-neutral-300 focus:ring focus:ring-neutral-200 focus:ring-opacity-50 rounded-md shadow-sm py-0 text-sm">
                                <option value="">全て</option>
                                <option v-for="(lost_type, index) in lost_types" :value="lost_type">{{ lost_type }}</option>
                            </select>
            ...
            <Pagination :search_cond="search_cond" route_name="admin.lost_product" :total="total"></Pagination>
            <div v-for="(item, key) in products.data" class="border-neutral-200 border-b flex flex-wrap justify-between items-center gap-2">
                ...
                <div class="text-center flex flex-wrap gap-2">
                    <button type="button" @click="modalProductOpen(item)"
                        class="inline-block items-center px-8 py-2 bg-green-500 rounded-md text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-700 focus:outline-none focus:border-green-700 focus:shadow-outline-green transition ease-in-out duration-150">
                        編集
                    </button>

                    <button type="button" @click="destroy_product_last(item.id)"
                        class="inline-block items-center px-8 py-2 bg-red-500 rounded-md text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-700 focus:outline-none focus:border-red-700 focus:shadow-outline-red transition ease-in-out duration-150">
                        削除
                    </button>
                </div>
            </div>            
...
export default {
    components: {Head, AppLayout, Dialog, DialogPanel, DialogTitle, TransitionChild, TransitionRoot, Pagination},
    props: {
        errors: Object,
    ...
    methods: {
        ...
        search() {
            this.form_search.get(route('admin.lost_product'));
        },
        clear() {
