            ...
            <div class="">
                <form @submit.prevent="submit()">
                        ...
                        <tbody class="border-t">
                            <tr class="border-b border-r border-l">
                                ...
                                <td class="font-light p-1 whitespace-nowrap">
                                    <input v-model="new_form.title" type="text"
                                        class="bg-neutral-50 border border-neutral-300 text-neutral-900 text-sm rounded-lg  block px-2 py-1.5 w-full focus:border-neutral-300 focus:ring focus:ring-neutral-200 focus:ring-opacity-50"
                    ...
                    <div class="text-center py-2">
                        <button type="submit"
                            class="w-32 bg-neutral-500 hover:bg-neutral-700 text-white text-sm font-normal py-2 px-5  rounded">
                            登録
                        </button>
                    </div>
                </form>
            ...
            <div class="text-center pb-2">
                ...
                <div class="flex mb-1">
                    <draggable class="w-full list-group" :list="categories.data" @change="log">
                        <div class="my-1 flex w-full items-center" v-for="element in categories.data" :key="element.id">
                            <div class="px-2">{{ element.id }}</div>
                            <div
                                class="flex-1 rounded-md text-left cursor-pointer border h-12 border-neutral-500 flex items-center hover:bg-neutral-100 focus:bg-neutral-100 gap-2">
                                <div class="flex-1 whitespace-nowrap h-full">
                                    <input v-model="element.title" type="text"
                                        class="bg-neutral-50 text-neutral-900 text-sm rounded-l-md border-neutral-300 block px-2 h-full w-full focus:border-blue-400 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                        placeholder="" required />
                                </div>
                                <div class="align-middle h-full flex items-center gap-1">
                                    <button @click.prevent="levelUp(element.id)"
                                        class="bg-neutral-700 rounded p-1 hover:bg-neutral-600">
                                        <ArrowLongUpIcon class="w-5 h-5 text-white block" />
                                    </button>
                                    <button @click.prevent="levelDown(element.id)"
                                        class="bg-neutral-700 rounded p-1 hover:bg-neutral-600">
                                        <ArrowLongDownIcon class="w-5 h-5 text-white block" />
                                    </button>
                                </div>
                                <button @click="destroy(element.id)"
                                    class="bg-red-500 hover:bg-red-700 text-white text-sm font-normal py-1 px-2 mr-2 rounded">削除</button>
                            </div>
                        </div>
                    </draggable>
                </div>
                <div class="p-1 flex justify-center">
                    <button @click.prevent="submit_order()" :class="{ 'opacity-25': order_form.processing }"
                        ...
                        <div class="inline-block">保存する</div>
                    </button>
                </div>
            </div>
        </div>
